...
class SongSearch {
    private final List<Song> songs = new Songs1().getSongs();
    ...
    void genresPlayed() {
        ...
        System.out.println("distinct genre count = " + genres.size());

        Map<String, Long> genreCount = songs.stream()
                .collect(groupingBy(Song::getGenre, counting()));
        System.out.println("genreCount = " + genreCount);
    }

    void search(String artist) {
        Optional<Song> result = songs.stream()
                .filter(song -> song.getArtist().equalsIgnoreCase(artist)) // ignore case
                .findFirst();
        if (result.isPresent()) {
...
class Songs1 {
    public List<Song> getSongs() {
        return List.of(
                ...
                new Song("Immigrant Song", "Led Zeppelin", "Rock", 1970, 484) // fixed capitalization
        );
    }
}